...
3. **复制过程中的原位改写**（相机 / 录机还在写这张卡）：引擎以**规划扫描时**观测的
   大小与修改时间为基准，开拷前先等到基准至少 2.5 秒旧（大于 FAT 的 2 秒时间戳粒度；
   每次运行只等一次），每个文件读源前核对当前元数据与基准一致（不一致 = 源自扫描后已变，
   该文件不拷、请重扫），拷完（或与既有目标比对完、或续传判定跳过前）再按**句柄**读一次
   元数据（按路径的查询可能被 SMB 客户端缓存应答），**任一读不到或不相等都判失败、不落位、
   不标已验证**；读到的字节数也必须等于开拷前的大小。于是拷贝期间的任何写入都落在比
   基准晚的时间槽里，包括预分配、等长的原位改写——前提是写者的文件系统在写入时就更新修改
   时间（macOS / Linux 本地与 Samba 都是）。**Windows 不保证**：写句柄关闭前修改时间可能
   不更新、远程文件的时间戳可能不正确；所以 OCard 在 Windows 上以 `FILE_SHARE_READ` 打开
   源文件——正有程序在写就直接打不开（可见失败、按占用重试），我们读着期间别人也拿不到
   写句柄。**挡不住**的：(a) 等长原位改写**并把修改时间改回旧值**的对抗性场景；(b) 非
   Windows 客户端从 Windows 共享读取正被别的机器写入的文件（共享模式由客户端决定，std
   不暴露）；(c) 旧版本清单没有记录修改时间的文件只按开拷快照判（有 `copy-baseline-degraded`
   告警）。闭合 (a)(b) 都要在发布前把源再完整读一遍算哈希，拷卡时间翻倍，与上面同一个理由
   **明确接受**。这些前提在拷卡完成屏的「本卡可格式化」旁**面向操作员**写明。
