...
3. **复制过程中的原位改写**（相机 / 录机还在写这张卡）：引擎在读源之前采集源的
   大小与修改时间，拷完（或与既有目标比对完）再读一次，**任一读不到或不相等都判失败、
   不落位、不标已验证**；读到的字节数也必须等于开拷前的大小。它挡得住正常的改写
   （长度变、或修改时间前进）。**挡不住**的是等长原位改写并把修改时间改回旧值的对抗性
   场景——那会产出一半旧一半新的目标文件并通过回读校验。闭合它要在发布前把源再完整
   读一遍算哈希，拷卡时间翻倍，与上面同一个理由**明确接受**。
