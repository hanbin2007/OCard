...
///
/// # 跨进程写同一份清单:由任务租约 + 写栅栏挡住(0.4.4)
///
/// 这是**整份覆盖**,不是读-改-写事务。0.4.4 起同一个 manifest id 由 `core::lease`
/// 的任务租约独占:续传第一件事是拿租约,别的进程被拒绝并可见地通知;每一次落盘都在
/// 写栅栏内([`save_guarded`] 在改名之前再问一句栅栏还在不在)。没有存储端 CAS 的
/// 残余边界——迟到的写入盖回旧清单——在 `core::lease` 模块头如实声明,并由清单旁的
/// 「不可信」标记(`suspect`)兜底。
...
pub const SAVE_FENCE_LOST: &str =
    "落盘前的最后一刻发现本任务的租约栅栏已不成立(读不到锁标记,或盘上租约已不是本任务的;可能是存储抖了一下,也可能是锁被外部回收 / 被别的进程接管;这里分不出来),这份清单没有发布;已停下,可续传";
